#!/usr/bin/env bash
### Created by Peter Forret ( pforret ) on 2023-01-15
...
readonly script_created="2023-01-15"
readonly run_as_root=-1 # run_as_root: 0 = don't check anything / 1 = script MUST run as root / -1 = script MAY NOT run as root
readonly script_description="Some customised Jekyll workflows"
## some initialisation
action=""
script_prefix=""
script_basename=""
...
Script:main() {
  ...
  case $action in
    new)
      #TIP: use «$script_prefix new» to create new post
...
do_new() {
  ...
  local title
  local slug
  day="$(IO:question "What date should the post be published?" "$(date '+%Y-%m-%d')")"
  year="${day:0:4}"
  month="${day:5:2}"
  IO:debug "$day -> $year -- $month"
  title="$(IO:question "What is the title of the post?" "The Title")"
  slug=$(Str:slugify "$title")

  IO:log "new post: $day - $title"
  # shellcheck disable=SC2154
  local image="$image_dir/$year/$month/$slug.jpg"
  local image_folder
  image_folder="$(dirname "$image")"
  [[ ! -d "$image_folder" ]] && mkdir -p "$image_folder"
  (
    [[ ! -f "$image" ]] && pushd "$image_folder" &>/dev/null && splashmark -e dark,grain -i "$title" unsplash cat "$(basename "$image")"
  )
  # shellcheck disable=SC2154
...
do_deploy() {
  ...
  Os:require jekyll_taxonomy "basher install pforret/jekyll_taxonomy"
  ...
  setver autopatch
  Tool:throughput "$script_started_at"
